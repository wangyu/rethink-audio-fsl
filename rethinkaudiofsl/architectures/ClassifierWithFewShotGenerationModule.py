...
class AttentionBasedBlock(nn.Module):
    ...
    def forward(self, features_train, labels_train, weight_base, Kbase):
        ...
        return weight_novel, AttentionCoeficients.data.cpu().numpy()
...
class Classifier(nn.Module):
    def __init__(self, opt):
        ...
        self.freeze_base_weight = opt['freeze_base_weight'] if ('freeze_base_weight' in opt) else False
    ...
    def get_classification_weights(
        ...
        if self.freeze_base_weight:
            self.weight_base.requires_grad = False
            self.bias.requires_grad = False
            self.scale_cls.requires_grad = False
        ...
        if features_train is None or labels_train is None:
            ...
            att_coeff = None
            return weight_base, att_coeff

        #***********************************************************************
        #******* Generate classification weights for the novel categories ******
        _, num_train_examples, num_channels = features_train.size()
        labels_train = labels_train[:, :, nKbase:]
        nKnovel = labels_train.size(2)
        if self.classifier_type in ['cosine', 'standard_cosine', 'featnorm', 'standard_featnorm']:
        ...
        if self.weight_generator_type=='none':
            ...
            weight_novel = self.wnLayerFavg(weight_novel_avg.view(batch_size * nKnovel, num_channels))
            weight_novel = weight_novel.view(batch_size, nKnovel, num_channels)
            att_coeff = None
        elif self.weight_generator_type=='attention_based':
            weight_novel_avg = self.favgblock(features_train, labels_train)
            weight_novel_avg = self.wnLayerFavg(weight_novel_avg.view(batch_size * nKnovel, num_channels))
            if self.classifier_type in ['cosine', 'standard_cosine']:
                weight_base_tmp = F.normalize(weight_base, p=2, dim=weight_base.dim()-1, eps=1e-12)
            else:
                weight_base_tmp = weight_base

            weight_novel_att, att_coeff = self.attblock(features_train, labels_train, weight_base_tmp, Kbase_ids)
    ...
    def apply_classification_weights(self, features, cls_weights):
        ...
        Return:
            cls_scores: A 3D tensor with shape
                ...
                categories.
        """
        if self.classifier_type in ['standard', 'standard_featnorm', 'standard_cosine']:
            if self.classifier_type == 'standard_featnorm':
                features = F.normalize(features, p=2, dim=features.dim() - 1, eps=1e-12)
            elif self.classifier_type == 'standard_cosine':
                features = F.normalize(features, p=2, dim=features.dim() - 1, eps=1e-12)
                cls_weights = F.normalize(cls_weights, p=2, dim=cls_weights.dim() - 1, eps=1e-12)
            cls_scores = torch.baddbmm(1.0, torch.zeros((1, 1, 1)).cuda(), 1.0, features, cls_weights.transpose(1, 2))
        else:
            if self.classifier_type=='cosine':
                features = F.normalize(features, p=2, dim=features.dim()-1, eps=1e-12)
                cls_weights = F.normalize(cls_weights, p=2, dim=cls_weights.dim()-1, eps=1e-12)
            elif self.classifier_type=='featnorm':
                features = F.normalize(features, p=2, dim=features.dim()-1, eps=1e-12)

            cls_scores = self.scale_cls * torch.baddbmm(1.0, self.bias.view(1, 1, 1),
                                                        1.0, features, cls_weights.transpose(1,2))
    ...
    def forward(self, features_test, Kbase_ids, features_train=None, labels_train=None):
        ...
        """
        cls_weights, att_coeff = self.get_classification_weights(Kbase_ids, features_train, labels_train)
        cls_scores = self.apply_classification_weights(features_test, cls_weights)

        if labels_train is None:
            novel_weights = None
        else:
            nKbase = Kbase_ids.shape[1]
            labels_train = labels_train[:, :, nKbase:]
            nKnovel = labels_train.shape[2]
            novel_weights = cls_weights[:, -nKnovel:, :].data.cpu().numpy()

        return cls_scores, att_coeff, novel_weights
