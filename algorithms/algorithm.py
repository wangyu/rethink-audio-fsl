...
class Algorithm():
    def __init__(self, opt):
        ...
        self.curr_epoch = 0
        self.optimizers = {}
        self.writer = SummaryWriter(comment=opt['exp_dir'].split('/')[-1])

        self.keep_best_model_metric_name = (opt['best_metric'] if ('best_metric' in opt) else None)

        self.schedule_lr = (opt['schedule_lr'] if ('schedule_lr' in opt) else False)
        self.step_per_batch = (opt['step_per_batch'] if ('step_per_batch' in opt) else 1)
    ...
    def init_network(self, net_def_file, net_opt, pretrained_path, key):
        ...
        print('number of trainable params:', key, sum(p.numel() for p in network.parameters() if p.requires_grad))
        return network
    ...
    def load_pretrained(self, network, pretrained_path):
        ...
        if 'module' in list(pretrained_model['network'].keys())[0]:
            self.logger.info('==> Model was saved with nn.DataParallel')
    ...
    def init_all_optimizers(self):
        ...
        # init lr scheduler if using it
        if self.schedule_lr:
            self.schedulers = {}
            for key, oparams in self.optim_params.items():
                self.schedulers[key] = None
                if oparams != None:
                    self.schedulers[key] = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizers[key], 'min')
        else:
    ...
    def init_optimizer(self, net, optim_opts, key):
        ...
        weight_decay = (self.opt['criterions']['loss']['l2reg'] if ('l2reg' in self.opt['criterions']['loss']) else 0)
        optimizer = None
        parameters = filter(lambda p: p.requires_grad, net.parameters())
        self.logger.info('Initialize optimizer: %s with params: %s for netwotk: %s'
            % (optim_type, optim_opts, key))
        if optim_type == 'adam':
            optimizer = torch.optim.Adam(parameters, lr=learning_rate, betas=optim_opts['beta'], weight_decay=weight_decay)
        elif optim_type == 'sgd':
    ...
    def init_all_criterions(self):
        ...
        for key, val in criterions_defs.items():
            ...
            crit_l1reg = val['l1reg'] if ('l1reg' in val) else None
            self.logger.info('Initialize criterion[%s]: %s with options: %s, %s' %
                             (key, crit_type, crit_opt, crit_l1reg))
    ...
    def init_criterion(self, ctype, copt):
        ...
        elif self.opt['criterions']['loss']['opt'] == 'pos_weight':
            nKbase = self.opt['criterions']['loss']['nKbase']
            nKnovel = self.opt['criterions']['loss']['nKnovel']
            pos_weight = torch.tensor([[1]*(nKbase-nKnovel) + [0.1]*nKnovel])
    ...
    def load_network(self, net_key, epoch,suffix=''):
        ...
        if os.path.isfile(filename):
            ...
            if list(checkpoint['network'].keys())[0][:6] == 'module':
                self.logger.info('==> Model was saved with nn.DataParallel')
                self.networks[net_key] = torch.nn.DataParallel(self.networks[net_key])
        ...
        for self.curr_epoch in range(start_epoch, self.max_num_epochs):
            ...
            if data_loader_test is not None:
                ...
                for key, val in eval_stats.items():
                    self.writer.add_scalar('val'+key, val, self.curr_epoch)
                self.keep_record_of_best_model(eval_stats, self.curr_epoch)

                # reduce lr if using lr scheduler and if val loss is plateauing
                if self.schedulers is not None:
                    for key in self.schedulers:
                        if self.schedulers[key] is not None:
                            self.schedulers[key].step(eval_stats['loss'])
                            print('scheduled lr: ', self.optimizers[key].param_groups[0]['lr'])
    ...
    def run_train_epoch(self, data_loader, epoch):
        ...
        for idx, batch in enumerate(tqdm(data_loader(epoch))):
            ...
            for n in range(self.step_per_batch):
                train_stats_this = self.train_step(batch)
                train_stats.update(train_stats_this)
                if (idx+1) % disp_step == 0:
                    avg_train_stats = train_stats.average()
                    self.logger.info('==> Iteration [%3d][%4d / %4d]: %s' %
                                     (epoch+1, idx+1, len(data_loader()), avg_train_stats))
                    for key, val in avg_train_stats.items():
                        self.writer.add_scalar(key, val, epoch*len(data_loader())+idx)
    ...
    def keep_record_of_best_model(self, eval_stats, current_epoch):
        if self.keep_best_model_metric_name is not None:
            ...
            if self.min_loss_val is None or metric_val < self.min_loss_val:
                self.min_loss_val = metric_val
                self.best_stats = eval_stats
                self.save_checkpoint(
                    self.curr_epoch+1, suffix='.best', metric=self.min_loss_val)
